using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;
using SpellCardsGenerator.Data.Data;
using SpellCardsGenerator.Data.Entities;
using SpellCardsGenerator.Data.Repositories.Abstract;

namespace SpellCardsGenerator.Data.Repositories;

public sealed class SpellLevelContentRepository : GlobalizedEntityRepository<int, SpellLevelContent, SpellCardsDataContext>
{
  public SpellLevelContentRepository(
    ILogger<SpellLevelContentRepository> logger,
    SpellCardsDataContext context)
    : base(logger, context)
  { }
}